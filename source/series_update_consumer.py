		...
		while True:
			try:
				...
				if job is None:
					...
					existing_chapters = mangadb.sourceDb.chapters.find( { "series_id" : series_id } )
					existing_chapter_urls = Set([c["url"] for c in existing_chapters])

					series_contents = requests.get(common.base_url + series_record["url"]).text
					psr = parser(series_contents)
					chapters = self.chapter_scraper.get_chapter_urls(psr);
					if chapters is None:
						common.logger.info("Didnt receive any chapters for %s", series_name)
					elif existing_chapters is not None and len(chapters) <= existing_chapters.count():
						common.logger.info("No new chapters for %s", series_name)
					else:
						new_chapters = self.find_new_chapters(chapters, existing_chapter_urls);
						common.logger.info("Got %s new chapters for %s", len(new_chapters), series_name)
